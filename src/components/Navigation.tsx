      ...
      >
        <div
          className={`mx-auto max-w-7xl px-6 flex items-center justify-between transition-all duration-500 ${
            scrolled ? "glass-panel rounded-2xl py-3 px-6 mx-4" : ""
          }`}
        >
          {/* Logo */}
          <Link to="/" className="flex items-center gap-3 group">
            <div className="relative w-8 h-8">
              <div className="absolute inset-0 rounded-xl glass-panel-sm flex items-center justify-center">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            >
              ...
              >
                <Globe className="w-3.5 h-3.5 transition-colors duration-300" style={{ color: "hsl(var(--primary))" }} />
              </motion.div>
            ...
            <Link to="/contact">
              <button className="btn-glass-primary text-xs">{t("nav.cta")}</button>
            </Link>
          </div>
          ...
          <div className="md:hidden flex items-center gap-2">
            <motion.button
              ...
              whileTap={{ scale: 0.93 }}
            >
              <motion.div animate={globeSpinning ? { rotateY: 360 } : { rotateY: 0 }} transition={{ duration: 0.6, ease: "easeInOut" }}>
                <Globe className="w-3 h-3" style={{ color: "hsl(var(--primary))" }} />
              </motion.div>
              <span className="font-display text-[9px] tracking-widest font-bold uppercase" style={{ color: "hsl(var(--foreground))" }}>
                {language === "en" ? "EN" : "PT"}
              </span>
            </motion.button>
            ...
            >
              <AnimatePresence mode="wait">
                {theme === "dark" ? (
                  <motion.div key="sun-m" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} transition={{ duration: 0.25 }}>
                    <Sun className="w-3.5 h-3.5" style={{ color: "hsl(var(--primary))" }} />
                  </motion.div>
                ) : (
                  <motion.div key="moon-m" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} transition={{ duration: 0.25 }}>
            ...
            <button className="glass-panel-sm p-2" onClick={() => setMobileOpen(!mobileOpen)}>
              {mobileOpen ? (
              ...
              {navLinks.map((link, i) => (
                <motion.div key={link.href} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
                  <Link
                    ...
                    style={{ color: location.pathname === link.href ? "hsl(var(--primary))" : "hsl(var(--foreground))" }}
                  >
                    {link.label}
                  </Link>
                </motion.div>
